...
\renewcommand{\flhead}{\vskip5pt \fboxsep=0pt {\small\bfseries\footnotesize \Sexpr{df$full_cat[i]} | \Sexpr{df$full_cat_ukr[i]}}}
\renewcommand{\fcfoot}{\vskip5pt \fboxsep=0pt \hspace{2pt}{\small\bfseries\footnotesize \Sexpr{df$tier[i]}}}

\renewcommand{\blhead}{\vskip5pt {\small\bfseries\footnotesize \Sexpr{df$full_cat[i]} | \Sexpr{df$full_cat_ukr[i]} }}
...
\begin{tabular}{l l}
\textsc{sg}: & (frá) \Sexpr{df$ÞGFET[i]} -- (til) \Sexpr{df$EFET[i]} \\ 
\textsc{pl}: & (frá) \Sexpr{df$ÞGFFT[i]} -- (til) \Sexpr{df$EFFT[i]} \\
\end{tabular}}
}