...
lan <- "en."
    ...
    for (i in 1:nrow(df)){
            if (!df$OBEYGJANLEGT[i] == ""){
                out = c(out, knit_child('../templates/pdf/en/obeyg.rnw'))
            } else if (df$bin_tag[i] == "so"){
                out = c(out, knit_child('../templates/pdf/en/so.rnw'))
            } else if (df$bin_tag[i] == "afn"){
                out = c(out, knit_child('../templates/pdf/en/afn.rnw'))
            } else if (df$bin_tag[i] == "lo" & df$OBEYGJANLEGT[i] == ""){
                out = c(out, knit_child('../templates/pdf/en/lo.rnw'))
            } else if (df$bin_tag[i] == "ao"){
                out = c(out, knit_child('../templates/pdf/en/ao.rnw'))
            } else if (df$bin_tag[i] %in% c('kk', 'kvk', 'hk', 'pfn')){
                out = c(out, knit_child('../templates/pdf/en/no_pfn.rnw'))
            } else if (df$bin_tag[i] %in% c('fn', 'gr', 'rt', 'to')){
                out = c(out, knit_child('../templates/pdf/en/fn_gr_to.rnw'))
            } else {
                print(df[i,])
        }
