...
for (i in 1:nrow(df)){
        if (!df$OBEYGJANLEGT[i] == ""){
            out = c(out, knit_child('../templates/pdf/pl/obeyg.rnw'))
        } else if (df$bin_tag[i] == "so"){
            out = c(out, knit_child('../templates/pdf/pl/so.rnw'))
        } else if (df$bin_tag[i] == "afn"){
            out = c(out, knit_child('../templates/pdf/pl/afn.rnw'))
        } else if (df$bin_tag[i] == "lo" & df$OBEYGJANLEGT[i] == ""){
            out = c(out, knit_child('../templates/pdf/pl/lo.rnw'))
        } else if (df$bin_tag[i] == "ao"){
            out = c(out, knit_child('../templates/pdf/pl/ao.rnw'))
        } else if (df$bin_tag[i] %in% c('kk', 'kvk', 'hk', 'pfn')){
            out = c(out, knit_child('../templates/pdf/pl/no_pfn.rnw'))
        } else if (df$bin_tag[i] %in% c('fn', 'gr', 'rt', 'to')){
            out = c(out, knit_child('../templates/pdf/pl/fn_gr_to.rnw'))
        } else {
            print(df[i,])
    }
